...
export class JogoDaVelhaService {
 ...
 jogar(posX: number, posY: number):void{
   ...
   if(!this.vitoria && this.numMovimentos < 9) this.cpuJogar();
    ...
    if(jogada.length<=0){
      ...
      for(let i=0; i<this.TAM_TB; i++){
        for(let j=0; j<this.TAM_TB; j++){
          if(this.tabuleiro[i][j]=== this.VAZIO) jogadas.push([i,j]);
        }
      }
  ...
  obterJogada(jogador: number): number[] {
    ...
    for(let lin; lin<this.TAM_TB; lin++){
      for(let col; col<this.TAM_TB; col++){
        if(tab[lin][col]!==this.VAZIO)continue;
        tab[lin][col]=jogador;
        if(this.fimJogo(lin,col,tab,jogador))return [lin,col];
